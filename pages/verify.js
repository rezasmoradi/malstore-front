...
function Verify() {
    ...
    const verifyCode = async () => {
        const res = await axiosClient.post('/auth/verify', { email: email, code: otpCode })
        if (res.status === 201) {
            document.cookie = `token=${res.data.token_type} ${res.data.token}; expires=${new Date(res.data.expire_at).toUTCString()}; path=/`
    ...
    return (
        <div className="w-full h-screen transition-all duration-300 dark:bg-slate-900">
                ...
                <div className="flex flex-col-reverse md:flex-row justify-around border-2 dark:border-slate-600 rounded-lg py-4 shadow-md shadow-slate-300 dark:shadow-slate-600 backdrop-blur-md">
                    ...
                    <div className="w-11/12 mx-auto md:mx-0 md:w-2/5 h-96 relative">
                        <div className="w-full h-40">
                            ...
                            <TextField inputClassName="text-center" label="" value={otpCode} onChange={(e) => { setOtpCode(e.target.value.trim()) }} />
                        </div>
